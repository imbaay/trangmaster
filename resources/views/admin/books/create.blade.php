...
@section('content')
    <div class="container-fluid">
        ...
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">Add new phone</h1>

        <!-- DataTales Example -->
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Phone create form</h6>
            </div>
            <div class="card-body">
                {!! Form::open(['method'=>'POST', 'action'=>'Admin\AdminPhonesController@store', 'files'=>true]) !!}
                ...
                <div class="form-group">
                    {!! Form::label('noisanxuat_id', 'Nơi sản xuất') !!}
                    {!! Form::select('noisanxuat_id', App\Noisanxuat::pluck('name', 'id'), null,['placeholder'=>'Select noisanxuat','class'=>'form-control '.($errors->has('noisanxuat_id')? 'is-invalid': '')]) !!}
                    @if($errors->has('noisanxuat_id'))
                        <span class="invalid-feedback">
                            <strong>{{$errors->first('noisanxuat_id')}}</strong>
                        </span>
                    @endif
                </div>
                ...
                <div class="form-group">
                    ...
                    {!! Form::select('danhmuc_id', App\Danhmuc::pluck('name', 'id'), null,['placeholder'=>'Select category','class'=>'form-control '.($errors->has('danhmuc_id')? 'is-invalid': '')]) !!}
                    @if($errors->has('danhmuc_id'))
                        <span class="invalid-feedback">
                            <strong>{{$errors->first('danhmuc_id')}}</strong>
                        </span>
                    @endif
                </div>
